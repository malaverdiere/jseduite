...
public class AbsenceFinder {
     ...
     */
    @WebMethod(operationName = "findAbsenceById")
    public Absence findAbsenceById(@WebParam(name = "id") int id)
            throws TeacherAbsenceException {

        String sql = "SELECT * FROM `teacher_absences`";
        sql += "WHERE `id` = '" + id + "';";
    ...
    /**
     * Extract all persistent references for the Absence business object
     * @return all existing absence references
     * @throws AbsenceException
     */
    @WebMethod(operationName="getAllAbsenceReferences")
    public int[] getAllAbsenceReferences() throws TeacherAbsenceException {
        DataAccessLayer dal = new DataAccessLayer();
        try {
            String sql = "SELECT `id` FROM `teacher_absences`;";
