...
public class AlarmAdmin {
   ...
   @WebMethod(operationName = "createAlarm")
    ...
    String content) throws AlarmsAdminException, DALException{
         ...
         String sql="insert into `alarms` values(NULL,'"+content+"');";
         dal.executeVoid(sql);
    ...
    int alarmId) throws DALException {
       ...
       int breakId) throws DALException {
         ...
         dal.executeVoid(sql);
         sql="update alarms set content='"+content+ "'where `alarm_id`='"+alarmId+"';";
         dal.executeVoid(sql);
         if(breakId!=0){
             sql="insert into break_alarm_lnk values('"+breakId+"','"+alarmId+"');";
             dal.executeVoid(sql);
    ...
    /**
     * update an alarm
     * @param alarmid,breakid,content
     * @return void
     */
    @WebMethod(operationName = "updateAlarm")
    public void updateAlarm(@WebParam(name = "alarmid")
    int alarmid, @WebParam(name = "breakid")
    int breakid, @WebParam(name = "content")
